...
- A held-out experiment compresses the 95-line skill into a ~60-token generic paragraph (no benchmark-specific hints) and re-runs the same 5 tasks across all 3 models with prompt caching enabled on every skill arm. Result: **Opus parity** (39 vs 38 LOC), **Haiku 2.00× gap** (78 vs 39), **Sonnet 2.36× gap** (99 vs 42). The cliff is between frontier and sub-frontier — not a smooth gradient. The paragraph still wins cost (−41%) on every model — structural, not behavior-tuned. The skill's 95 lines aren't filler; they're rule redundancy that catches what sub-frontier models ignore when stated once. See [`results/2026-06-14-oneliner-vs-skill.md`](results/2026-06-14-oneliner-vs-skill.md) and `arms/oneline.js`.
